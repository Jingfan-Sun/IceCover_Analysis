...
close(gcf);
%% Read data
D = importdata('unified-sea-ice-thickness-cdr-1947-2012/USSUB-AN_summaries_1975_2005_v1.txt');
[row, col] = size(D.data);

%% Find same campaign
campaign_Name = cell(1, col); % save different names in the data
campaign_Name{1} = D.data(1, 1);
campaign_Index = 1; % save the start index of different campaign in campaign_name variable
campaign_Year = D.data(1, 3);
categary_Index = 1;
for i = 3: (row + 1)
    if(campaign_Name{categary_Index} ~= D.data(i-1, 1))
       categary_Index = categary_Index + 1;
       campaign_Name{categary_Index} = D.data(i-1, 1); 
       campaign_Index = [campaign_Index, i - 1];
       campaign_Year = [campaign_Year, D.data(i-1, 3)];
    end
end
...
for i = 1:length-1
   ...
   hleg1 = text(0.7,(0.6 - room*i), [num2str(i), ' ', num2str(campaign_Name{i}), ' ', num2str(campaign_Year(i))], 'Color', color_Temp, 'FontSize', 8, 'fontweight','bold', 'fontname','Nimbus Sans L');
...
title = title('USSUB-AN_summaries_1975_2005_v1_1', 'fontweight','bold','fontsize',12,'fontname','Nimbus Sans L');
set(title,'Interpreter','none');

print(gcf, '-dpng', '-r300' ,'USSUB-AN_summaries_1975_2005_v1_1.png');
