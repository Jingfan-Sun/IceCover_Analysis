...
close all;
%% Read data
D = importdata('../Step3/unified-sea-ice-thickness-cdr-1947-2012/CanCoast_summaries_1947_2010_v1.txt');
...
for i = 1: length - 1
    ...
    for j = 1: numel(x)
        ...
        yearCounter = campaign_Year{1, i}(data_Position(j));
        % Find the nearst date in 5-day-long
        monthCounter = month2str(floor(Yday(campaign_Index(i) + data_Position(j) - 1)/ 31) + 1);
        date_All{1, j} = date;
        % Read NC File
        srcP = '/mnt/storage0/myers/NEMO/ORCA025-GLORYS2V3/'; % 4th
        ...
        % Find neighbour grid points
        [result,index]=sort((lat_G(:) - lat(campaign_Index(i))).*(lat_G(:) - lat(campaign_Index(i))) + (lon_G(:) - lon(campaign_Index(i))).*(lon_G(:) - lon(campaign_Index(i))));
        inverseDistance = zeros(1,4);
        kk = 1; % Number of valid neighbour points
        k = 1; % Number of index
        valid_Index = zeros(1, 4);
        while kk <= 4
            if(~isnan(iceC(index(k))))
                inverseDistance(1,kk) = 1 / sum((m_ll2xy(lon(campaign_Index(i)), lat(campaign_Index(i))) - m_ll2xy(lon_G(index(k)), lat_G(index(k)))) .^ 2);
                valid_Index(1, kk) = index(k);
                kk = kk + 1;
            end
            % Break active situations:
        ...
        sum_Distance = sum(inverseDistance);
        for k = 1:kk-1
            track_G(j) = track_G(j) + iceC(valid_Index(k)) * inverseDistance(1, k) / sum_Distance;
        end
    end
    hold on;
    p4 = plot(date_X, track_G, '-g', 'LineWidth', 2);
    grid on;
    set(gca,'fontweight','bold','fontsize',7,'fontname','Nimbus Sans L');
