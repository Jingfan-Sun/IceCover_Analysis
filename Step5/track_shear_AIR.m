...
close all;
%% Read data
D = importdata('../Step3/unified-sea-ice-thickness-cdr-1947-2012/AIR-EM_summaries_2001_2009_v1.txt');
...
e1t = GetNcVar(meshhgr,'e1t',[0 0 0],[1632 2400 1]);
e2t = GetNcVar(meshhgr,'e2t',[0 0 0],[1632 2400 1]);
...
for i = 2: length - 1
	...
	disp([num2str(j), '/', num2str(campaign_Index(i + 1) - campaign_Index(i))]);
        ...
        for k = 1:7
            ...
            v_y = zeros(NY - 2, NX);
            for jjj = 1: NX - 2
                du(:, jjj) = u(:, jjj + 2) - u(:, jjj);
                u_x(:, jjj) = du(:, jjj) ./ e1t(:, jjj + 1) ./ 2;
                u_y(:, jjj) = du(:, jjj) ./ e2t(:, jjj + 1) ./ 2;
            end
            for jjj = 1: NY - 2
                dv(jjj, :) = v(jjj + 2, :) - v(jjj, :);
                v_x(jjj, :) = dv(jjj, :) ./ e1t(jjj + 1, :) ./ 2;
                v_y(jjj, :) = dv(jjj, :) ./ e2t(jjj + 1, :) ./ 2;
            end
            du = du(2: NY - 1, :);
            u_x = u_x(2: NY - 1, :);
            u_y = u_y(2: NY - 1, :);
            ...
            v_x = v_x(:, 2: NX - 1);
            v_y = v_y(:, 2: NX - 1);
            shear_deformation = shear_deformation + sqrt(((u_x - v_y) .^ 2) + ((u_y + v_x) .^ 2));
        end
    ...
    subplot(312);
    p5 = plot(distance_X, track_shear, '-r', 'LineWidth', 3, 'LineStyle', ':');
    grid on;
    set(gca,'fontweight','bold','fontsize',7,'fontname','Nimbus Sans L');
    ...
    ncfile=[srcP,'GLORYS2V3_ORCA025_20110115_R20130808_icemod.nc']; % 4th
    
    lon_G=GetNcVar(ncfile,'nav_lon',[0 0],[1438 1020]);
    lat_G=GetNcVar(ncfile,'nav_lat',[0 0],[1438 1020]);
    
    track_G = zeros(size(x)); % data from ANHA4
    for j = 1: (campaign_Index(i + 1) - campaign_Index(i))
        % Calculate the data
        yearCounter = campaign_Year(i);
        % Find the nearst date in 5-day-long
        monthCounter = month2str(floor(Yday(campaign_Index(i) + j - 1)/ 31) + 1);
        % Read NC File
        srcP = '/mnt/storage0/myers/NEMO/ORCA025-GLORYS2V3/'; % 4th
        ncfile=[srcP,'GLORYS2V3_ORCA025_',num2str(yearCounter),monthCounter,'15_R20130808_icemod.nc']; % 4th
        ...
        % Find neighbour grid points
        [result,index]=sort((lat_G(:) - lat_Temp(j)).*(lat_G(:) - lat_Temp(j)) + (lon_G(:) - lon_Temp(j)).*(lon_G(:) - lon_Temp(j)));
        inverseDistance = zeros(1,9);
        for k = 1:9
            if(~isnan(iceC(index(k))))
                inverseDistance(1,k) = 1 / sum((m_ll2xy(lon_Temp(j), lat_Temp(j)) - m_ll2xy(lon_G(index(k)), lat_G(index(k)))) .^ 2);
            else
                inverseDistance(1,k) = 0;
            end
        end
        ...
        for k = 1:9
            if(inverseDistance(1,k) ~= 0)
                track_G(j) = track_G(j) + iceC(index(k)) * inverseDistance(1, k) / sum_Distance;
            end
        end
    end
    subplot(311);
    hold on;
    p4 = plot(distance_X, track_G, '-g', 'LineWidth', 2);
    grid on;
    set(gca,'fontweight','bold','fontsize',7,'fontname','Nimbus Sans L');
    
    disp(['GLORYS finished ', num2str(i)]);
    %% title
    ...
    legend([p1 p2 p3 p4], 'Observation', 'ANHA4', 'ANHA12', 'GLORYS2V3', 'Location', 'NorthEast');
    ...
    clear title xlabel ylabel;
    xlabel('Distance /km');
    ylabel('Shear Deformation');
    
    xLimit = get(gca, 'XLim');
    xLimit_up = round(xLimit(2) / 3.5);
    set(gca, 'XLim', [0, xLimit(2) + xLimit_up]);
    legend(p5, 'Shear Deformation', 'Location', 'NorthEast');
    % Calculate the position of each points in global coordinate
    x1 = zeros(1, xLimit(2) + xLimit_up);
    y1 = zeros(1, xLimit(2) + xLimit_up);
    YLim_plot = get(gca, 'YLim');
    ...
    %% Map plot
    subplot(313);
    lon_12 = lon_12(2: 2400 - 1, 2: 1632 - 1);
    lat_12 = lat_12(2: 2400 - 1, 2: 1632 - 1);
    ...
    %         end
    %     else
    m_line(lon_Temp, lat_Temp, 'linewi',2, 'Color', 'g');
    load mycolormap
    ...
    set(gca, 'CLim', [0 1e-5]); % ice thickness 0-5.0 \\\ ice concentrstion -0.35-0.35
    ...
    for j = 1: (campaign_Index(i + 1) - campaign_Index(i))
        if(j == 1 || j == (campaign_Index(i + 1) - campaign_Index(i)))
            ...
            text(x, y, num2str(j), 'Color', 'r', 'FontSize', 4, 'fontweight','bold', 'fontname','Nimbus Sans L');
    ...
    % available
    if(rem(numel(hxlabel), 2) ~= 0)
        delete(hxlabel([1 round(numel(hxlabel)/2-1)]));
        delete(hxlabel([round(numel(hxlabel)/2)+1 numel(hxlabel)]));
    else
        delete(hxlabel([1 round(numel(hxlabel)/2)-1]));
        delete(hxlabel([round(numel(hxlabel)/2)+2 numel(hxlabel)]));
    end
