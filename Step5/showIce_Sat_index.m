...
for control_Index = 5: 10
    
    if nargin==0
        flname=['../Step2/icesat_icethk_', name_Array{control_Index}, '_filled.dat'];
    end
    ...
    monthCounter = month2str(floor(timeCounter / 31) + 1);
    ...
    for i = 1:sat_SizeX
        for j = 1:sat_SizeY
            ...
            elseif(myIceH(i, j) == 0)
                ...
                [result,index]=sort((lat_G(:) - sat_Lat(i, j)).*(lat_G(:) - sat_Lat(i, j)) + (lon_G(:) - sat_Lon(i, j)).*(lon_G(:) - sat_Lon(i, j)));
                % [result,index]=sort(abs(lat_G(:) - temp_Lat) + abs(lon_G(:) - temp_Lon));
                % calculate distance of 4 neighbour points
                neighbour_Index{i, j} = index(1:4, 1);
                distance = zeros(1,4);
                for k = 1:4
                    distance(1,k) = sum((m_ll2xy(sat_Lon(i, j), sat_Lat(i, j)) - m_ll2xy(lon_G(index(k)), lat_G(index(k)))) .^ 2);
                end
                sum_Distance = sum(distance);
                for k = 1:4
                    model_Compare(i, j) = model_Compare(i, j) + iceC(index(k)) * distance(1, k) / sum_Distance;
                end
                flag = flag + 1;
    ...
    save(['icesat_icethk_', num2str(control_Index), '_GLORYS_index'], 'neighbour_Index');
    save(['icesat_icethk_', num2str(control_Index), '_GLORYS_data'], 'model_Compare');
