...
for yearCounter = 2007
    ...
    for i = 1:day_Num
        ...
        iceC_12=GetNcVar(ncfile,data_Type,[subII(1)-1 subJJ(1)-1 0],[numel(subII) numel(subJJ) 1]);
        disp(num2str(i));
        ...
        % read ice concentration
        iceC_4=GetNcVar(ncfile,data_Type,[subII(1)-1 subJJ(1)-1 0],[numel(subII) numel(subJJ) 1]);
        disp(num2str(i));
        iceC = iceC_12(1:3:2398, 1:3:1630) - iceC_4;
        ...
        if isProj==0
            ...
            set(gca, 'CLim', [-0.35, 0.35]); % ice thickness 0-5.0 \\\ ice concentrstion -0.35-0.35
            % set(gca, 'CLimMode', 'auto');
            set(hbar,'position',[0.9 0.1055 0.02 0.8203]);
            
            set(gcf, 'visible', 'on');
            xlabel(['Ice Concentration Diff on ', date],'fontweight','bold','fontsize',12,'fontname','Nimbus Sans L');
            print(gcf, '-dpng', '-r300' ,['Ice_Concentration_Diff_', date,'.png']);
