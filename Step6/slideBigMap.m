...
m_gshhs_i('patch',[0 0 0]); set(findobj('tag','m_coast'),'linestyle','none'); % low resolution coastlines
m_nolakes;

% plot map grid
m_grid('box','fancy','tickdir','in','bac','none','xtick',myxtick,'ytick',myytick,'linestyle','-','linewidth',1,'fontsize',18)
...
hxlabel=findobj(gca,'tag','m_grid_xticklabel');  set(hxlabel,'fontweight','b','fontsize',8,'fontname','Nimbus Sans L','rotation',0');
hylabel=findobj(gca,'tag','m_grid_yticklabels'); set(hylabel,'fontweight','b','fontsize',8,'fontname','Nimbus Sans L','rotation',0');
delete(hxlabel(1))
delete(hylabel([2 4 5 6 7]));
[xxtmp,yytmp]=m_ll2xy((85:-5:45)*0-15,85:-5:45);

movePostion(hylabel(1),0.02,'y');
movePostion(hylabel(3),0.02,'y');
%    for np=1:2:9
